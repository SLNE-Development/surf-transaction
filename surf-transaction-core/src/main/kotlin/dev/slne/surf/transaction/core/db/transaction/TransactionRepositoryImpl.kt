...
class TransactionRepositoryImpl : TransactionRepository {
    ...
    suspend fun persistTransaction0(transaction: TransactionImpl): TransactionResult {
        val senderID =
            transaction.senderAccountId?.let { AccountRepository.findAccountIDByAccountId(it) }
        val receiverID =
            transaction.receiverAccountId?.let { AccountRepository.findAccountIDByAccountId(it) }
        val currencyID = CurrencyRepository.findCurrencyIDByName(transaction.currencyName)
    ...
    override suspend fun balanceDecimal(accountId: UUID, currency: Currency): BigDecimal =
        suspendTransaction {
            val accountID =
                AccountRepository.findAccountIDByAccountId(accountId) ?: error("Account not found")
            val currencyID = CurrencyRepository.findCurrencyIDByName(currency.name)
                ?: error("Currency not found")
